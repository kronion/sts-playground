...
# Download data here: https://drive.google.com/file/d/1R5eyUebTXsNJV4PCoDQE9lWfKxlWmsNs/
DATA = flags.DEFINE_string("data", None, "path to data", required=True)
...
def main(_):
    ...
    total_size = len(tree.flatten(column_major["state_before"])[0])
    ...
    # dm-tree considers numpy arrays to be leaves of the structure,
    # whereas lists are intermediate nodes. We need to convert the
    # action list to a numpy array so it will be mapped correctly
    # with the rest of the structure.
    column_major["action"] = np.array(column_major["action"])
    ...
    # TODO this is wrong because the action space is also part of the input size
    input_size = space_size(base.OBSERVATION_SPACE)
    print("input_size", input_size)

    obs_space = space_as_nest(base.OBSERVATION_SPACE)
    action_space = space_as_nest(base.ACTION_SPACE)
    overall_space = {"state_before": obs_space, "action": action_space, "state_after": obs_space}
    print("num components:", len(tree.flatten(overall_space)))

    # # Separate the previous states from the next states
    # train_states_after = train_tensor.pop("state_after")
    # valid_states_after = valid_tensor.pop("state_after")
    ...
    input_obs_dim = space_size(base.OBSERVATION_SPACE)
    input_action_dim = space_size(base.ACTION_SPACE)
    input_dim = input_obs_dim + input_action_dim
    ...
    def get_loss(batch: tp.Sequence[torch.Tensor]):
        batch = tree.unflatten_as(overall_space, batch)
        breakpoint()
        # x = list(map(encode, flat_spaces, batch))
        flat_obs_input = encode(base.OBSERVATION_SPACE, batch["state_before"])
        flat_action_input = encode(base.ACTION_SPACE, batch["action"])
        flat_input= torch.cat((flat_obs_input, flat_action_input), dim=-1)

        # TODO change autoencoder input dimension to add in action
        flat_output = auto_encoder.forward(flat_input)
        if LOSS.value == "ce":
            losses = struct_loss(base.OBSERVATION_SPACE, flat_output, batch["state_after"])
        ...
        top1 = accuracy(base.OBSERVATION_SPACE, flat_output, batch["state_after"])
